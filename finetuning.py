...
with open(f"logs/training_log_{num_epoch}_{batch_size}.txt", "w") as log_file:
    for epoch in range(num_epoch):
        ...
        os.makedirs("models/set5", exist_ok=True)
        os.makedirs(f"models/set5/progress_{num_epoch}_{batch_size}", exist_ok=True)
        checkpoint = {
            'epoch': epoch + 1,
        ...
        torch.save(checkpoint, f"models/set5/progress_{num_epoch}_{batch_size}/checkpoint_epoch_{epoch+1}.pth")
        
# 学習曲線を保存
os.makedirs("logs/set5", exist_ok=True)
pd.DataFrame({"loss": loss_list, "val_loss": val_loss_list}).plot()
plt.savefig(f"logs/set5/loss_{num_epoch}_{batch_size}.png")

# 学習済みモデルの保存
torch.save(model.state_dict(), f"models/set5/finetuned_clip_model_{num_epoch}_{batch_size}.pth")
